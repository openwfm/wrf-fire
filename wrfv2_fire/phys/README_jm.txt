...
Each module is it its own .F file. All files are in wrf/wrfv2_fire/phys.

core_test and prop_test use their own simplified spread function and so
they do not refer to the physics in any way. model_test uses the actual
Rothermel's formulas from module_fr_sfire_speed. Visualization in testers
is by Matlab. Instructions are provided when running the tester.

ifmake is a script for the Intel compiler. There is also g95make for g95.
...
Everybody uses util. Other than that, use of modules is permitted only
along the arrows above. For example, model may not call a subroutine
from prop, and prop may not use an array declared in params. This is
to make possible the development and testing of the math algorithms
...
independent of WRF. Fake versions of some WRF procedures are linked in
the testers.

The code currently violates some WRF conventions, esp. uses heap memory,
and will not run in parallel. This will be fixed later, after the whole
coupled atmosphere-fire model runs correctly.

How to call model from atm be clear from model_test_main.F.
...
It is planned to replace prop by a better algorithm in future.
What is in there works but may not be accurate and is not optimized.
For example, the current version seems to add a bit of spread rate even
