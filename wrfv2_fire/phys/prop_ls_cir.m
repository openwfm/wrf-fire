...
while t<time1-tol & istep < msteps
    ...
    switch method
        case 'cir'
            dt=min(min(dx./max(abs(speed.*nvx),eps)))+min(min(dy./max(abs(speed.*nvy),eps)));
            dt=min(time1-t,0.5*dt)
            % locations back on characteristics, in mesh index coordinates
            [xii,yii]=meshgrid(1:m,1:n);
            xinc=dt.*speed.*nvx./dx;
            xi = xii - xinc;
            yinc=dt.*speed.*nvy./dy;
            yi = yii - yinc; 
            phi_new=interp2(xii,yii,phi,xi,yi);
            phi=min(phi_new,phi);
        case 'godunov'
            switch split
            case 'wind'
