...
% mesh is from fd(1) to fd(2), so its length is fd(2)-fd(1)
if (fd(2)<fd(1))
      display('fuel_burnt_fd mesh size should be >0, but fd(2)-fd(1)<0 ');
else
    if all(lfn(:)>=0),
...
% u(3)=t(h/2,h/2)-h/2(dt/dx+dt/dy)
 ...
 dt_dx=(tign(2,1)-tign(1,1)+tign(2,2)-tign(1,2))/(2*msh_sz);
 dt_dy=(tign(1,2)-tign(1,1)+tign(2,2)-tign(2,1))/(2*msh_sz);
 u(1)=dt_dx;
 u(2)=dt_dy;
 u(3)=tign_middle-msh_sz*(dt_dx+dt_dy)/2;
        if all(lfn(:)<=0),
    % all burning
    % integrate
    uu = -u / fuel_time;
...
%    = 1 if a==0
% intexp=1/b*integral(0,b) of exp(ax)dx
if eps < abs(ab)^3/6,
