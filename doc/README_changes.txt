...
subgrid support by changes to the files
...
to them than what normally would be.
...
To get the changes that John made: git diff wrfsvn wrfsubgrid or a
summary all modifications from the bare subgrid version git diff
--stat wrfsubgrid origin/master
...
Registry/Registry.EM - new section to declare fire variables and
halos phys/module_physics_addtendc.F - add the output of fire code
to heat and moisture tendency dyn_em/solve_em.F - call the fire
driver phys/Makefile - new entries for fire model Makefile - new
target em_fire
...
In our testing so far the fire code takes about 20% of CPU. The
coupled WRF-fire code scales just as well as WRF itself.
...
The fire code does need to have data inputs, at the subgrid resolution.
...
standard data sources.  We are importing this data through a modified
(to support subgrids) version of WPS, and WRF imports it from
wrfinput_d?? just like every other data source.  The fuel category
data we are currently using comes from this website:
...
imported in the standard way.  The repository we have given you
