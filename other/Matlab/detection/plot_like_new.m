%function plot_like_new
        ...
        % function h=heat(T)
        c = 2;  % time constant (h) : how long the heat takes to drop to exp(-1)
        %********************************
        heat = @(T) (T<=0).*exp(min(T,0)/c);
        %********************************
        % T = fire arrival time
        % 0 if T > T_now = 0, 1 at T=0, exp decay for T>= 0
        figure(1);fplot(@(T)heat(-T),[-c,5*c]);
        xlabel('time since fire arrival (h)');ylabel('heat fraction (1)')
        title('Burn model')
        fig
        ...
        xlabel('heat fraction (1)');ylabel('probability of detection (1)');
        title('Logistic fire detection model')
        fig
        
        figure(3);fplot(@(T) prob(heat(-T)),[-c,10*c]);
        xlabel('time since fire arrival (h)');ylabel('probability of detection (1)');
        title('Fire detection probability')
        fig
        ...
        sigma = 2000,     % geolocation error stdev
        ...
        xlabel('time since fire arrival (h)');ylabel('log probability of detection (1)');
        title('Log likelihood with geolocation error')
        fig
