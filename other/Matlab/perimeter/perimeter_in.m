...
for ts=time:-1:2
   if size(A,1)==0
         ...
         for dx=-1:1
            for dy=-1:1
               ...
               if (C(A(i,1)+dx,A(i,2)+dy)==0) 
                  ...
                  F=0.25*interval*(ros_old(A(i,1),A(i,2),2-dx,2-dy)+ros_new(A(i,1),A(i,2),2-dx,2-dy) + ...
                    ros_old(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy)+ros_new(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy)); 
                  if (I(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy)+F>distance(A(i,1),A(i,2),2-dx,2-dy))
                     tign(A(i,1)+dx,A(i,2)+dy)=(ts)*interval-((distance(A(i,1),A(i,2),2-dx,2-dy)-I(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy))/F)*interval;
                     C(A(i,1)+dx,A(i,2)+dy)=1;
                     D(A(i,1)+dx,A(i,2)+dy)=tign(A(i,1)+dx,A(i,2)+dy)-(ts-1)*interval;
                     if (D(A(i,1)+dx,A(i,2)+dy)<0)
                        data_steps=sprintf('%s\n Error: D happens to be less than 0',data_steps);
                     end                           
                  else                        
                     I(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy)=F+I(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy);
                  end                       
               elseif (C(A(i,1)+dx,A(i,2)+dy)==1)
                  F=0.25*interval*(ros_old(A(i,1),A(i,2),2-dx,2-dy)+ros_new(A(i,1),A(i,2),2-dx,2-dy) + ...
                    ros_old(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy)+ros_new(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy));                     
                  if (I(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy)+F>distance(A(i,1),A(i,2),2-dx,2-dy)) 
                     tign_new=(ts)*interval-((distance(A(i,1),A(i,2),2-dx,2-dy)-I(A(i,1)+dx,A(i,2)+dy,2-dx,2-dy))/F)*interval;
                     if tign_new>tign(A(i,1)+dx,A(i,2)+dy)
      ...
      for a=1:size(A,1)    
         if ~any(any(C(A(a,1)-1:A(a,1)+1,A(a,2)-1:A(a,2)+1)==0))
            C(A(a,1),A(a,2))=2;
         end
      end
      ...
      [tign,C,D,I,data_steps]=get_tign_one_timestep(tign,ros_old,ros_new,C,D,I,distance,interval,ts,data_steps);
...
function [tign,C,D,I,data_steps]=get_tign_one_timestep(tign,ros_old,ros_new,C,D,I,distance,interval,ts,data_steps)
         ...
         for dy=-1:1
            if (C(B(j,1)+dx,B(j,2)+dy)==0) 
               ...
               if (F>distance(B(j,1),B(j,2),2-dx,2-dy))
                  tign(B(j,1)+dx,B(j,2)+dy)=tign(B(j,1),B(j,2))- ...
                    ((distance(B(j,1),B(j,2),2-dx,2-dy))/F)*(D(B(j,1),B(j,2)));
            ...
            elseif (C(B(j,1)+dx,B(j,2)+dy)==1)
               ...
               if (F>distance(B(j,1),B(j,2),2-dx,2-dy))
                  ...
                  I(B(j,1)+dx,B(j,2)+dy,2-dx,2-dy)=F+I(B(j,1)+dx,B(j,2)+dy,2-dx,2-dy);
               end                       
            end
         end    
      end
