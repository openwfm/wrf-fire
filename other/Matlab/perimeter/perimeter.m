...
[A,tign]=tign_update(long,lat,tign,A,IN,V,time_now);

changed=sum(tign(:)~=tign_last(:));
fprintf('step %i tign changed at %i points\n',istep,changed)
%figure(1),mesh(tign_last),title('tign last (outside)')
figure(2),mesh(tign-tign_last),title('Difference (outside)')
figure(3),mesh(tign),title('tign new (outside)')
...
changed=sum(tign(:)~=tign_last(:));
fprintf('step %i tign changed at %i points\n',istep,changed)
%figure(4),mesh(tign_last),title('tign last')
figure(5),mesh(tign-tign_last),title('Difference')
figure(6),mesh(tign),title('tign new')
...
function [A,tign]=tign_update(long,lat,tign,A,IN,V,time_now)
                ...
                for a=a1:a2   
                    for b=b1:b2  
                        ...
                        if (A(a,b)==1) % was already updated 
                        	tign_new=tign(a,b)-sqrt((long(a,b,1)-long(i,j,1))^2+ ...
                                     (lat(a,b,1)-lat(i,j,1))^2)/                 ...
                                     R_wind(dot([lat(a,b,1)-lat(i,j,1),          ... 
                                     long(a,b,1)-long(i,j,1)],V(:,i,j)));
                ...
                for a=a1:a2  
                	for b=b1:b2  
                    	if (A(a,b)==1)                                
                        	tign_new=tign(a,b)+sqrt((long(a,b,1)-long(i,j,1))^2+ ...
                                     (lat(a,b,1)-lat(i,j,1))^2)/                 ...
                                     R_wind(dot([long(i,j,1)-long(a,b,1),        ... 
                                     lat(i,j,1)-lat(a,b,1)],V(:,a,b)));
